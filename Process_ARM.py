...
def compute_thermo(ds, pbl_thickness_ratio, pmid):
    ...
    thetae_bl = _bl_ave(ds.thetae, pbl_top, p_near_surf)
    thetae_lft = _lft_ave(ds.thetae, pbl_top, pmid)
    thetae_sat_lft = _lft_ave(ds.thetae_sat, pbl_top, pmid)
...
def _bl_ave(var, pbl_top:xr.DataArray, pbl_bottom:xr.DataArray)->xr.DataArray:

    """Returns average over the boundary layer
...
def _lft_ave(var, pbl_top:xr.DataArray, pmid:xr.DataArray)->xr.DataArray:

    """Returns average over the lower-free troposphere
